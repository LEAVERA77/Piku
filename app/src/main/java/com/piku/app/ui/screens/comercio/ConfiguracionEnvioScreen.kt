        ...
        when {
            cargando -> CircularProgressIndicator(Modifier.padding(padding).padding(32.dp))
                ...
                Row(
                    ...
                    Switch(checked = realizaEnvios, onCheckedChange = { realizaEnvios = it })
                }
                if (realizaEnvios) {
                        ...
                        Row(
                            Modifier
                                ...
                                .selectable(
                                    ...
                                    onClick = { modoEnvio = modo },
                                    role = Role.RadioButton
                                )
                                .padding(vertical = 4.dp),
                            verticalAlignment = Alignment.CenterVertically
                        ) {
                            RadioButton(selected = modoEnvio == modo, onClick = { modoEnvio = modo })
                    ...
                    if (modoEnvio == ModoEnvio.CON_COSTO || modoEnvio == ModoEnvio.GRATIS_DESDE_MONTO) {
                        ...
                        OutlinedTextField(
                            value = costoEnvio,
                            onValueChange = { costoEnvio = it.filter { c -> c.isDigit() || c == '.' || c == ',' } },
                    ...
                    if (modoEnvio == ModoEnvio.GRATIS_DESDE_MONTO) {
                        ...
                        OutlinedTextField(
                            value = minimoCompra,
                            onValueChange = { minimoCompra = it.filter { c -> c.isDigit() || c == '.' || c == ',' } },
